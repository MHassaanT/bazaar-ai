...
import { Zap, Clock, TrendingUp, TrendingDown, Minus, Trash2, ArrowRight, Loader2 } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    if (user) {
      ...
      Promise.all([
        api.get('/api/users/me').catch(err => {
          console.error('Error fetching user profile:', err);
          return { data: { credits_remaining: 15, credits_total: 15, credits_used: 0, plan: 'free' } };
        }),
        api.get('/api/history/my').catch(err => {
        ...
        .catch(err => {
          ...
          // Set fallback values
          setProfile({ credits_remaining: 15, credits_total: 15, credits_used: 0, plan: 'free' });
          setHistory([]);
        })
  ...
  const creditsRemaining = profile ? (profile.credits_total - (profile.credits_used || 0)) : 15;
  const creditsTotal = profile?.credits_total || 15;
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6 shadow-xl sticky top-28">
          ...
          <div className="bg-[#060b18] rounded-xl p-4 border border-white/5 mb-6 text-center">
            {isPro ? (
              ...
              <>
                <h3 className="text-white font-medium flex items-center justify-center gap-1 mb-2"><Zap className="w-4 h-4 text-blue-400 fill-current" /> {creditsRemaining} / {creditsTotal} Credits</h3>
                <div className="w-full bg-white/10 h-2 rounded-full mb-4 overflow-hidden">
                  <div className="bg-blue-500 h-full rounded-full transition-all duration-300" style={{ width: `${(creditsUsed / creditsTotal) * 100}%` }}></div>
                </div>
          ...
          <div className="flex flex-col gap-4 pb-20">
            {history.map((item, i) => (
              <motion.div
                ...
                key={item.id}
                className="group relative bg-[#0d1526] hover:bg-[#121c33] transition-colors border border-white/5 rounded-xl p-5 flex flex-col md:flex-row items-center gap-4 shadow-sm hover:shadow-xl hover:shadow-blue-900/10 cursor-pointer"
              ...
              >
                ...
                <div className="flex flex-wrap gap-2 md:gap-4 md:items-end w-full md:w-auto mt-4 md:mt-0">
                  <div className="flex flex-col gap-1 items-start md:items-end">
                    <span className={`text-xs px-2 py-0.5 rounded font-medium ${item.demand_label && item.demand_label.includes('High') ? 'bg-green-500/20 text-green-400' : item.demand_label && item.demand_label.includes('Low') ? 'bg-red-500/20 text-red-400' : 'bg-amber-500/20 text-amber-400'}`}>Demand: {item.demand_label || 'N/A'}</span>
                    <span className="flex items-center text-xs text-gray-400 gap-1">{item.demand_trend && item.demand_trend.includes('Rising') ? <TrendingUp className="w-3 h-3 text-green-400" /> : item.demand_trend && item.demand_trend.includes('Declining') ? <TrendingDown className="w-3 h-3 text-red-400" /> : <Minus className="w-3 h-3" />} {item.demand_trend || 'N/A'}</span>
                  </div>
                  <button
                    onClick={(e) => { e.stopPropagation(); handleDelete(item.id) }}
                    className="md:ml-4 text-gray-500 hover:text-red-500 transition-colors p-2 rounded-full hover:bg-red-500/10 self-center"
                  >
