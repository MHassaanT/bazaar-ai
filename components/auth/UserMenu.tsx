...
export default function UserMenu() {
  ...
  const [creditsText, setCreditsText] = useState('15 / 15');
      ...
      api.get('/api/users/me')
        .then(res => {
          ...
          if (data.plan === 'pro') {
            ...
            const remaining = (data.credits_total || 15) - (data.credits_used || 0);
            setCreditsText(`${Math.max(0, remaining)} / ${data.credits_total || 15}`);
          }
        })
        .catch(error => {
          console.error('Error fetching user profile:', error);
          // Set default values if API fails
          setCreditsText('15 / 15');
          setProfileData({
